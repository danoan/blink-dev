...
def test_database(DB_URL):
	with connect_database(DB_URL) as conn:
		with conn.cursor() as cur:
			sql="Select name from activity where id = %s"
			cur.execute(sql,(12,))
...
def insert_user(cur,**a):
	...
	try:
		...
		if cur.rowcount==0:
			sql = "INSERT INTO blinker(name,age,city,country,location,gender,fbid,rating) VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id"
...
def insert_international_by_code(cur,**a):
	...
	try:
		sql_0 = "SELECT blinker_id FROM international_blinker WHERE blinker_id=%s"
		data_0 = (a["blinker_id"],)
		...
		sql_2 = "INSERT INTO international_blinker (blinker_id,document_1_id,document_2_id,contact_1_id,contact_2_id,status) VALUES (%s,%s,%s,%s,%s,%s)"
